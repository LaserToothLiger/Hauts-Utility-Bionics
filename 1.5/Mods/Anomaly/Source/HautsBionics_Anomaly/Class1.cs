...
namespace HautsBionics_Anomaly
{
    ...
    public class HautsBionics_Anomaly
    {
        ...
        static HautsBionics_Anomaly()
        {
            ...
            {
                harmony.Patch(AccessTools.Method(typeof(PregnancyUtility), nameof(PregnancyUtility.ApplyBirthOutcome_NewTemp)),
                               postfix: new HarmonyMethod(patchType, nameof(HVB_ApplyBirthOutcome_NewTempPostfix)));
            }
        }
        ...
        public static void HVB_ApplyBirthOutcome_NewTempPostfix(ref Thing __result, Thing birtherThing)
        {
    ...
    public class IncidentWorker_HomunculusExamined : IncidentWorker
    {
        ...
        {
            //base.SendStandardLetter(this.def.letterLabel, this.def.letterText, this.def.letterDef, parms, null, Array.Empty<NamedArgument>());
            return true;
        }
    }
    ...
    public class HediffComp_VoidshardDownside : HediffComp
    {
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            if (this.causativeHediffs == null || this.causativeHediffs.Count == 0)
            {
                ...
                if (this.Pawn.IsHashIntervalTick(250) && ((Find.Anomaly.Level <= 0 || Find.Anomaly.Level > 5) || (this.Pawn.Faction != null && (this.Pawn.Faction == Faction.OfEntities || this.Pawn.Faction == Faction.OfHoraxCult))))
                {
                    this.Pawn.health.RemoveHediff(this.parent);
                }
            }
    ...
    public class HediffComp_Corruptor : HediffComp
    {
        ...
        {
            base.CompPostTick(ref severityAdjustment);
            if ((this.Pawn.story == null || !this.Pawn.story.traits.HasTrait(HVBAnomalyDefOf.HVB_HomunculusTrait)) && this.Pawn.IsHashIntervalTick(150) && this.Pawn.health.immunity.DiseaseContractChanceFactor(this.Props.hediff, this.parent.Part) > 0.001f && Rand.MTBEventOccurs(Math.Max(this.Props.corruptionMtbDays - (this.Props.mtbLossPerExtraSeverity*this.parent.Severity),0.001f), 60000f, 150f) && !this.Pawn.health.hediffSet.HasHediff(this.Props.hediff))
            {

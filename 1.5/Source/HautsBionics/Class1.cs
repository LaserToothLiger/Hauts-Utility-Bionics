...
namespace HautsBionics
{
    ...
    /*public class Recipe_IED : Recipe_Surgery
    {
        public override void ApplyOnPawn(Pawn pawn, BodyPartRecord part, Pawn billDoer, List<Thing> ingredients, Bill bill)
        {
            if (billDoer != null)
            ...
            HediffComp_IED hcied = hediff.TryGetComp<HediffComp_IED>();
            if (hcied != null)
            {
                foreach (Thing t in ingredients)
                {
                    if (t.def.thingCategories.Contains(DefDatabase<ThingCategoryDef>.GetNamed("MortarShells")))
                    {
                        hcied.shellType = t.def;
                        break;
                    }
                }
            }
    ...
    public class Recipe_NoIED : Recipe_Surgery
    {
        ...
        public override void ApplyOnPawn(Pawn pawn, BodyPartRecord part, Pawn billDoer, List<Thing> ingredients, Bill bill)
        {
            ...
            if (billDoer != null)
            {
                ...
                if (hediff != null)
                {
                    ...
                    if (hcied != null)
                    {
                        hcied.SetToDefaultShellType();
                        GenSpawn.Spawn(hcied.shellType, billDoer.Position, billDoer.Map, WipeMode.Vanish);
                    }
    ...
    public class CompAbilityEffect_EMPDischarge : CompAbilityEffect_Nova
    {
        ...
        {
            base.AffectSelf();
            GenExplosion.DoExplosion(this.parent.pawn.PositionHeld, this.parent.pawn.Map, Math.Min(this.Props.baseRadius * this.parent.pawn.BodySize, this.Props.maxRadius), DamageDefOf.EMP, null, -1, -1f, null, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0f, false, null, null, null, true, 1f, 0f, true, null, 1f);
        }
        public override bool VictimCounter()
        {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            if (target.Thing != null)
            {
                if (target.Thing.def.category != ThingCategory.Building && target.Thing.def.category != ThingCategory.Plant && MassUtility.FreeSpace(this.parent.pawn) >= target.Thing.GetStatValue(StatDefOf.Mass))
                {
                    ...
                    this.DoLink(target.Thing, target.Thing, this.parent.pawn.Position, this.parent.pawn.Map, ticksToDisappear);
    ...
    public class GrabFlyer : Thing, IThingHolder
    {
        ...
        {
            GenDraw.DrawLineBetween(this.other.Position.ToVector3(), this.DrawPos, AltitudeLayer.Conduits.AltitudeFor(), this.Rope, 0.25f);
    ...
    public class Hediff_DosageSustainer : Hediff_AddedPart
    {
        ...
        {
            base.PostTick();
            if (this.pawn.IsHashIntervalTick(240))
            {
    ...
    public class HediffComp_ShieldGenerator : HediffComp
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            if (this.Pawn.IsHashIntervalTick(120) && this.Props.bionicShieldDef != null && this.Props.makesShield && !this.Pawn.health.hediffSet.HasHediff(this.Props.bionicShieldDef))
            {
    ...
    /*public class HediffCompProperties_IED : HediffCompProperties
    {
        public HediffCompProperties_IED()
        {
            this.compClass = typeof(HediffComp_IED);
        }
        public ThingDef defaultShellType;
        public float toxResistantChanceForDefaultToxShell;
        public IntRange ticksToExplosion;
    }
    public class HediffComp_IED : HediffComp
    {
        public HediffCompProperties_IED Props
        {
            get
            {
                return (HediffCompProperties_IED)this.props;
            }
        }
        public override string CompLabelInBracketsExtra { 
            get {
                return this.shellType.label;
            } 
        }
        public override void CompPostPostAdd(DamageInfo? dinfo)
        {
            base.CompPostPostAdd(dinfo);
            this.SetToDefaultShellType();
        }
        public override void Notify_PawnKilled()
        {
            ...
            {
                ExplosionTimer thing = (ExplosionTimer)GenSpawn.Spawn(HVBDefOf.HVB_IEDExplosionTimer, this.Pawn.PositionHeld, this.Pawn.Map, WipeMode.Vanish);
                thing.ticksToBoom = this.Props.ticksToExplosion.RandomInRange;
                thing.shellType = this.shellType;
                CompExplosive cexp = thing.TryGetComp<CompExplosive>();
                if (cexp != null)
                {
                    GenExplosion.NotifyNearbyPawnsOfDangerousExplosive(thing, cexp.Props.explosiveDamageType, null, null);
                }
                this.Pawn.health.RemoveHediff(this.parent);
            }
        }
        public void SetToDefaultShellType()
        {
            if (this.shellType == null)
            {
                if (ModsConfig.BiotechActive && this.Pawn.GetStatValue(StatDefOf.ToxicResistance) >= 0.9f && Rand.Chance(this.Props.toxResistantChanceForDefaultToxShell))
                {
                    this.shellType = ThingDefOf.Shell_Toxic;
                } else {
                    this.shellType = this.Props.defaultShellType;
                }
            }
        }
        ...
        {
            base.CompExposeData();
            Scribe_Defs.Look<ThingDef>(ref this.shellType, "shellType");
        }
        public ThingDef shellType;
    }
    public class ExplosionTimer : Thing
    {
        public override void PostMake()
        {
            ...
            if (this.shellType == null)
            {
                this.shellType = ThingDefOf.Shell_HighExplosive;
                CompExplosive cexp = this.TryGetComp<CompExplosive>();
                if (cexp != null)
                {
                    GenExplosion.NotifyNearbyPawnsOfDangerousExplosive(this, cexp.Props.explosiveDamageType, null, null);
                }
            }
        }
        public override void Tick()
        {
            if (this.ticksToBoom <= 0)
            {
                this.Detonate();
                return;
            }
            if (this.overlayBurningWick == null)
            {
                this.overlayBurningWick = new OverlayHandle?(this.Map.overlayDrawer.Enable(this, OverlayTypes.BurningWick));
            }
            this.ticksToBoom--;
        }
        public void Detonate()
        {
            Thing thing = ThingMaker.MakeThing(this.shellType);
            if (this.PositionHeld != null)
            {
                IntVec3 pos = this.PositionHeld;
                GenSpawn.CheckMoveItemsAside(pos, this.shellType.defaultPlacingRot, this.shellType, this.Map);
                GenSpawn.Spawn(thing, pos, this.Map, WipeMode.Vanish);
                CompExplosive cexp = thing.TryGetComp<CompExplosive>();
                if (cexp != null)
                {
                    cexp.StartWick(null);
                    MethodInfo methodInfo = typeof(CompExplosive).GetMethod("Detonate", BindingFlags.NonPublic | BindingFlags.Instance);
                    methodInfo.Invoke(cexp, new object[] { this.MapHeld, false });
                }
            }
            this.Destroy();
        }
        public override void ExposeData()
        {
            base.ExposeData();
            Scribe_Defs.Look<ThingDef>(ref this.shellType, "shellType");
            Scribe_Values.Look<int>(ref this.ticksToBoom, "ticksToBoom", 0, false);
        }
        public ThingDef shellType;
        public int ticksToBoom;
            ...
            get
            {
                ...
                if (this.pawn.Spawned && this.pawn.Map != null)
                {
                    ...
                    if (this.pawn.Position.GetRoom(this.pawn.Map) != null && !this.pawn.Position.GetRoom(this.pawn.Map).PsychologicallyOutdoors)
                    {
                        ...
                        num -= (Find.WorldGrid[this.pawn.Map.Tile].biome.diseaseMtbDays / 30f);
                    }
                    return num;
                }
                if (this.pawn.Tile != -1)
                {
                    num -= (1 + Find.WorldGrid[this.pawn.Tile].biome.diseaseMtbDays / 30f);
                }
                return num;
            }
        }
        ...
        public static void HVB_LandingEffects(PawnFlyer pawnFlyer)
        {
            if (pawnFlyer.FlyingPawn != null)
            {
                ...
                if (earthshakerCount > 0f)
                {
                    ...
                    GenExplosion.DoExplosion(pawnFlyer.Position, pawnFlyer.Map, radius, DamageDefOf.Smoke, null, -1, -1f, null, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0f, false, null, null, null, true, 1f, 0f, true, null, 1f);
                }
            }
        }
